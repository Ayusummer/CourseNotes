/*
 * @Author: your name
 * @Date: 2021-04-11 11:16:14
 * @LastEditTime: 2021-04-11 12:07:16
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \junior-lessons_second-term\EmbeddedSystem\Experiment\实验3-按键输入实验\v1.0.1-呼吸灯-百分比-周期数增加.c
 */
#include "sys.h"
#include "delay.h"
#include "usart.h"
      ...
      switch(key){
        ...
        case KEY0_PRES:
          while(1){
            ...
            for(i=breath_times_max; i>=breath_left_times; i--){        // 周期总数递减->从亮到暗
              // 若按下KEY1或者KEY1已经按下(flag==1)则熄灯并跳出呼吸循环
              if(led_off(KEY_Scan(1),flag))   break;  
              led_blink(breath_single_time, breath_single_time*i/breath_times_max);  //呼吸灯闪烁
            }

            // 若按下KEY1或者KEY1已经按下(flag==1)则熄灯并跳出呼吸循环
            if(led_off(KEY_Scan(1),flag))   break;  
            
            for(i=breath_left_times;i<=breath_times_max;i++){        // 周期总数自增->75,从暗到亮
              // 若按下KEY1或者KEY1已经按下(flag==1)则熄灯并跳出呼吸循环
              if(led_off(KEY_Scan(1),flag))   break;  
              led_blink(breath_single_time,breath_single_time*i/breath_times_max);  //呼吸灯闪烁
            }
